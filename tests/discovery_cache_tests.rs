...
async fn test_device_capabilities_and_metadata() {
    ...
    assert_eq!(
        device.metadata.get("version"),
        Some(&"12.3.4.5".to_string())
    );
    assert_eq!(
        device.metadata.get("location"),
...
async fn test_cache_expiration() {
    let config = DiscoveryCacheConfig {
        default_ttl: Duration::from_millis(10), // Even shorter TTL
        method_ttls: std::collections::HashMap::new(), // Clear method-specific TTLs
        ..create_test_config()
    };
    ...
    // Wait for expiration (wait much longer than TTL)
...
async fn test_cache_method_specific_ttls() {
    ...
    let unknown_device =
        create_test_device("192.168.1.102", "12347", "Unknown Device", "bluetooth");

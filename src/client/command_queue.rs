...
impl CommandQueue {
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        info!(
            "Command queue started with config: max_size={}, max_concurrent={}",
        ...
        for priority in &priorities {
            if let Some(queue) = queues.get_mut(priority) {
                while let Some(command) = queue.pop_front() {
                    ...
                    debug!(
                        "Dequeued command {} with priority {:?}",
            ...
            async move {
                ...
                if result.success {
                    ...
                    if retry_command.should_retry() {
                        debug!(
                            "Re-queuing command {} for retry (attempt {})",
                            command_id, retry_command.attempt_count
                        );
                        // Add delay for retry
        ...
        // Update statistics and store results
        {
            ...
            for result in &execution_results {
                ...
                if total_executions > 1 {
                    ...
                    let new_avg = ((current_avg * (total_executions - 1) as f64) + new_duration)
    ...
    async fn handle_expired_command(&self, command: QueuedCommand) {
        warn!(
            "Command {} expired (queued at {:?})",
            command.id, command.queued_at
        );
...
mod tests {
    ...
    async fn test_command_enqueue_dequeue() {
        ...
        let command =
            QueuedCommand::new("device1".to_string(), "on".to_string(), "test".to_string());
        let command_id = command.id;
    ...
    async fn test_command_priority_ordering() {
        ...
        let low_cmd =
            QueuedCommand::new("device1".to_string(), "low".to_string(), "test".to_string());

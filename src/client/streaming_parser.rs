...
impl StreamingStructureParser {
    ...
    pub async fn parse_from_response(&mut self, mut response: Response) -> Result<LoxoneStructure> {
        ...
        while let Some(chunk_result) = response.chunk().await.transpose() {
            ...
            let chunk = chunk_result
                .map_err(|e| LoxoneError::connection(format!("Stream read error: {e}")))?;
            ...
            if items_parsed % self.config.progress_interval == 0 {
                self.report_progress(bytes_processed, content_length, items_parsed)
                    .await;
            }
    ...
    async fn parse_global_states_section(
        &mut self,
        global_states: Map<String, Value>,
    ) -> Result<usize> {
        let mut count = 0;
        let max_items = if self.config.max_items_per_section > 0 {
            self.config.max_items_per_section
    ...
    async fn report_progress(
        ...
        let completion_percentage =
            total_bytes.map(|total| (bytes_processed as f32 / total as f32) * 100.0);

...
async def lifespan(mcp: FastMCP) -> AsyncGenerator[None, None]:
    """Manage server lifecycle - setup and cleanup."""
    global _context
    ...
    try:
        ...
        monitor_task = asyncio.create_task(loxone._monitor_connection())
        ...
        # Discover sensors dynamically (30 seconds of monitoring)
        ...
        # Cleanup
        if "_context" in locals() and _context and _context.loxone:
            try:
                # Cancel connection monitor if exists
    ...
    for device_uuid, device in _context.devices.items():
        if device.get("room") == room_uuid:
            device_info = {
                ...
                "room": _context.rooms.get(room_uuid, "Unknown")
            }
...
async def control_device(device: str, action: str, room: str | None = None) -> dict[str, Any]:
    ...
    if not _context.loxone.connected:
        ...
        try:
            await _context.loxone._ensure_connection()
        except Exception as e:
    ...
    for device_uuid, device_data in _context.devices.items():
        ...
        if (device.lower() in device_name or device == device_uuid):
            ...
            if room:
                ...
                if device_room_uuid:
                    room_name = _context.rooms.get(device_room_uuid, "").lower()
    ...
    try:
        if "jalousie" in device_type or "rolladen" in device_type:
            ...
            if action in ["up", "hoch", "rauf", "öffnen", "auf"]:
                result = await _context.loxone.send_command(f"jdev/sps/io/{target_uuid}/Up")
            elif action in ["down", "runter", "zu", "schließen"]:
                result = await _context.loxone.send_command(f"jdev/sps/io/{target_uuid}/Down")
            elif action in ["stop", "stopp", "anhalten"]:
                result = await _context.loxone.send_command(f"jdev/sps/io/{target_uuid}/stop")
            else:
            ...
            if action in ["on", "an", "ein"]:
                result = await _context.loxone.send_command(f"jdev/sps/io/{target_uuid}/On")
            elif action in ["off", "aus"]:
                result = await _context.loxone.send_command(f"jdev/sps/io/{target_uuid}/Off")
            else:
                return {"error": f"Invalid action '{action}' for light. Use: on, off"}

        else:
            # Generic control
            result = await _context.loxone.send_command(f"jdev/sps/io/{target_uuid}/{action}")
        ...
        if "connection" in str(e).lower() or "network" in str(e).lower():
            return {"error": f"Connection error: {e}. The system will attempt to reconnect automatically."}
...
async def get_devices_by_category(category: str | None = None) -> dict[str, Any]:
    ...
    if category is None:
        ...
        return {
            "available_categories": {
                ...
                for cat_name, devices in _context.devices_by_category.items()
            },
    ...
    results = {
        "category": category,
        "device_count": len(devices_in_category),
        "devices": []
    }
...
async def get_devices_by_type(device_type: str | None = None) -> dict[str, Any]:
    ...
    if device_type is None:
        ...
        return {
            "available_types": {
                dtype: len(devices)
                for dtype, devices in _context.devices_by_type.items()
            },
...
async def rediscover_sensors(discovery_time: float = 30.0) -> dict[str, Any]:
    ...
    try:
        ...
        # Discover sensors
        discovered_sensors = await discover_sensors_automatically(
            _context.loxone.websocket_client,
...
async def get_weather_data() -> dict[str, Any]:
    ...
    if not _context.capabilities.has_weather:
        return {
            ...
            "note": "Your Loxone system doesn't have weather stations or environmental sensors configured",
    ...
    for device_uuid, device in _context.devices.items():
        ...
        if any(keyword in device_name for keyword in weather_keywords):
            # Avoid duplicates
            if not any(d['uuid'] == device_uuid for d in weather_devices):
                weather_devices.append({
                    **device,
                    "discovery_method": "name",
                    "name_matched": device_name
                })
    ...
    for device in weather_devices:
        ...
        try:
            ...
            device_info["unit"] = _guess_weather_unit(device.get('name', ''), device.get('type', ''))
...
async def get_outdoor_conditions() -> dict[str, Any]:
    ...
    outdoor_devices = []
    outdoor_keywords = ["außen", "outdoor", "aussen", "outside", "garten", "garden", "terrasse", "terrace"]
    ...
    if conditions["humidity"]:
        ...
        conditions_parts.append(f"{humidity_info['value']}{humidity_info['unit']} humidity ({humidity_info['status']})")
    
    conditions["conditions_text"] = ", ".join(conditions_parts) if conditions_parts else "No outdoor sensor data available"
...
async def get_security_status() -> dict[str, Any]:
    ...
    security_keywords = ["alarm", "motion", "bewegung", "sensor", "überwach", "monitor", "tür", "door", "fenster", "window"]
    for device_uuid, device in _context.devices.items():
        device_name = device.get('name', '').lower()
        device_type = device.get('type', '')
        
        if (device_type in security_types and 
            any(keyword in device_name for keyword in security_keywords)):
            # Avoid duplicates
            if not any(d['uuid'] == device_uuid for d in security_devices):
                security_devices.append({
                    **device,
                    "discovery_method": "type_and_name",
                    "type_matched": device_type
                })
    ...
    elif (security_status["summary"]["doors_open"] > 0 or 
          security_status["summary"]["windows_open"] > 0):
...
async def get_energy_consumption() -> dict[str, Any]:
    ...
    for device_uuid, device in _context.devices.items():
        device_name = device.get('name', '').lower()
        if any(keyword in device_name for keyword in energy_keywords):
            if not any(d['uuid'] == device_uuid for d in energy_devices):
                energy_devices.append(device)
    ...
    energy_data = {
        ...
        "summary": {
            "total_power": 0,
            "total_energy": 0,
            "device_count": len(energy_devices)
        }
    }
    ...
    for device in energy_devices:
        ...
        try:
            ...
            if isinstance(state, (int, float)):
                if "watt" in device_info["unit"].lower() or "w" == device_info["unit"].lower():
...
async def get_audio_zones() -> dict[str, Any]:
    ...
    for device_uuid, device in _context.devices.items():
        device_name = device.get('name', '').lower()
        if any(keyword in device_name for keyword in audio_keywords):
            if not any(d['uuid'] == device_uuid for d in audio_devices):
                audio_devices.append(device)
...
def _guess_energy_unit(device_name: str, device_type: str) -> str:
...
async def get_climate_control() -> dict[str, Any]:
    ...
    climate_categories = ["Heizung", "Heating", "Klima", "Climate", "Lüftung", "Ventilation", "HVAC"]
    ...
    climate_keywords = ["heizung", "heating", "klima", "climate", "temp", "thermostat", "lüftung", "ventilation"]
    for device_uuid, device in _context.devices.items():
        device_name = device.get('name', '').lower()
        if any(keyword in device_name for keyword in climate_keywords):
            if not any(d['uuid'] == device_uuid for d in climate_devices):
                climate_devices.append(device)
    ...
    for device in climate_devices:
        ...
        try:
            ...
            device_info["current_state"] = state
            device_info["unit"] = _guess_climate_unit(device.get('name', ''), device.get('type', ''))
...
def _guess_climate_unit(device_name: str, device_type: str) -> str:
...
async def get_available_capabilities() -> dict[str, Any]:
    ...
    if capabilities.has_lighting:
        available_features["lighting"] = {
            ...
            "tools": ["control_device_by_uuid", "get_devices_by_type('LightController')", "find_devices('light')"],
    ...
    if capabilities.has_blinds:
        available_features["blinds_rolladen"] = {
            ...
            "tools": ["control_device_by_uuid", "get_devices_by_type('Jalousie')", "find_devices('rolladen')"],
    ...
    available_features["device_discovery"] = {
        ...
        "tools": ["discover_all_devices", "get_devices_by_category", "find_devices", "control_device_by_uuid"],
...
async def get_system_status() -> dict[str, Any]:
    ...
    top_categories = sorted(
        _context.devices_by_category.items(),
        key=lambda x: len(x[1]),
        reverse=True
    )[:5]

    top_device_types = sorted(
        _context.devices_by_type.items(),
        key=lambda x: len(x[1]),
        reverse=True
    )[:5]
    ...
    return {
        ...
        "detected_capabilities": {
            "lighting": {"available": _context.capabilities.has_lighting, "count": _context.capabilities.light_count},
            "blinds": {"available": _context.capabilities.has_blinds, "count": _context.capabilities.blind_count},
            "weather": {"available": _context.capabilities.has_weather, "count": _context.capabilities.weather_device_count},
            "security": {"available": _context.capabilities.has_security, "count": _context.capabilities.security_device_count},
            "energy": {"available": _context.capabilities.has_energy, "count": _context.capabilities.energy_device_count},
            "audio": {"available": _context.capabilities.has_audio, "count": _context.capabilities.audio_zone_count},
            "climate": {"available": _context.capabilities.has_climate, "count": _context.capabilities.climate_device_count}
        },
        "dynamic_sensors": {
    ...
    for category in light_categories:
        if category in devices_by_category:
            for device in devices_by_category[category]:
                if not any(device['uuid'] == d['uuid'] for d in devices_by_type.get("LightController", [])):
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("server_clean:mcp", host="0.0.0.0", port=8000)

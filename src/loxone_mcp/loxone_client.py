...
class Loxone:
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            raise LoxoneConnectionError(f"Failed to connect: {e}")
    ...
    async def authenticate(self) -> bool:
        ...
        try:
            ...
            logger.debug(f"Password preview: {self.password[:8]}..." if len(self.password) > 8 else "Password: (short)")
            
            # Check if password might be a token or special format
            if len(self.password) > 40:
                logger.info(f"Long password detected ({len(self.password)} chars) - might be a token or special format")
            ...
            if isinstance(auth_response, dict):
                ...
                logger.debug(f"Auth code: {auth_code}, Auth value: {auth_value}, Type: {type(auth_value)}")
            ...
            raise LoxoneAuthError(f"Authentication error: {e}")
        ...
        if self.structure is None:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    f"{self.http_url}/data/LoxAPP3.json",
    ...
    async def _send_command(self, command: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise LoxoneError(f"Command timeout: {command}")

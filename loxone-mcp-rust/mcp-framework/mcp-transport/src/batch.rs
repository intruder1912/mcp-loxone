    ...
    pub fn validate(&self) -> Result<(), TransportError> {
        match self {
            ...
            JsonRpcMessage::Batch(values) => {
                if values.is_empty() {
                    return Err(TransportError::Protocol("Batch cannot be empty".to_string()));
                }
...
pub async fn process_batch(
    ...
    debug!("Batch contains {} requests and {} notifications", 
           requests.len(), notifications.len());
    ...
    for request in requests {
        debug!("Processing request: {} (ID: {})", request.method, request.id);
    ...
    let response_message = if responses.len() == 1 && !matches!(message, JsonRpcMessage::Batch(_)) {
        ...
        // Batch response
        let response_values: Result<Vec<Value>, _> = responses.iter()
            .map(serde_json::to_value)
            .collect();
        
        let response_values = response_values
            .map_err(|e| TransportError::Protocol(format!("Failed to serialize batch response: {}", e)))?;

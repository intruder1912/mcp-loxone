...
use crate::{
    ...
    validation::{validate_message_string, extract_id_from_malformed},
    batch::{JsonRpcMessage, process_batch, create_error_response},
};
use async_trait::async_trait;
use mcp_protocol::Response;
...
impl StdioTransport {
    ...
    async fn process_line(
        ...
        if self.config.validate_messages {
            if let Err(e) = validate_message_string(line, Some(self.config.max_message_size)) {
                ...
                let error_response = create_error_response(
                    mcp_protocol::Error::invalid_request(format!("Message validation failed: {}", e)),
                    request_id,
                );
        ...
        let message = match JsonRpcMessage::parse(line) {
            ...
            Err(e) => {
                ...
                let request_id = extract_id_from_malformed(line);
                let error_response = create_error_response(
                    mcp_protocol::Error::parse_error(format!("Invalid JSON: {}", e)),
                    request_id,
                );
        ...
        if let Err(e) = message.validate() {
            ...
            let error_response = create_error_response(
                mcp_protocol::Error::invalid_request(format!("Invalid JSON-RPC: {}", e)),
        ...
        match process_batch(message, handler).await {
            ...
            Err(e) => {
                ...
                let error_response = create_error_response(
                    mcp_protocol::Error::internal_error(format!("Processing failed: {}", e)),
                    serde_json::Value::Null,
                );
    ...
    async fn send_line(
        ...
        if self.config.validate_messages {
            if let Err(e) = validate_message_string(line, Some(self.config.max_message_size)) {
                return Err(TransportError::Protocol(
                    format!("Outgoing message validation failed: {}", e)
                ));
            }
        }
        ...
        if let Err(e) = stdout.write_all(line_with_newline.as_bytes()).await {
            return Err(TransportError::Connection(format!("Failed to write to stdout: {}", e)));
        }
        
        if let Err(e) = stdout.flush().await {
            return Err(TransportError::Connection(format!("Failed to flush stdout: {}", e)));
        }
...
impl Transport for StdioTransport {
    ...
    async fn health_check(&self) -> Result<(), TransportError> {
        if self.running.load(std::sync::atomic::Ordering::Relaxed) {
            ...
            Err(TransportError::Connection("Transport not running".to_string()))
        }
    }
}
...
mod tests {
    use super::*;
    use mcp_protocol::{Request, Response, Error as McpError};
    use serde_json::json;
    use std::io::Cursor;

    // Mock handler for testing
    fn mock_handler(request: Request) -> std::pin::Pin<Box<dyn std::future::Future<Output = Response> + Send>> {
        Box::pin(async move {
    ...
    async fn test_health_check() {
        ...
        // Set as running
        transport.running.store(true, std::sync::atomic::Ordering::Relaxed);

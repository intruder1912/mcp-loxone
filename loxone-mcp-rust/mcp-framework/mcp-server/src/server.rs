...
use mcp_auth::{AuthConfig, AuthenticationManager};
use mcp_transport::{Transport, TransportConfig};
use mcp_security::{SecurityConfig, SecurityMiddleware};
use mcp_monitoring::{MonitoringConfig, MetricsCollector};
use mcp_protocol::*;
...
impl Default for ServerConfig {
    fn default() -> Self {
        Self {
            ...
            auth_config: mcp_auth::default_config(),
            transport_config: mcp_transport::TransportConfig::default(),
            security_config: mcp_security::default_config(),
            monitoring_config: mcp_monitoring::default_config(),
...
impl<B: McpBackend + 'static> McpServer<B> {
    ...
    pub async fn new(
        ...
        let transport = mcp_transport::create_transport(config.transport_config.clone())
    ...
    pub async fn start(&mut self) -> std::result::Result<(), ServerError> {
        ...
        self.backend.on_startup()
            .await
            .map_err(|e| ServerError::Backend(e.to_string()))?;
        
        // Start background services
        self.auth_manager.start_background_tasks()
            .await
            .map_err(|e| ServerError::Authentication(e.to_string()))?;
        ...
        self.transport.start(Box::new(move |request| {
    ...
    pub async fn stop(&mut self) -> std::result::Result<(), ServerError> {
        ...
        self.auth_manager.stop_background_tasks()
            .await
            .map_err(|e| ServerError::Authentication(e.to_string()))?;
...
pub use mcp_monitoring::ServerMetrics;
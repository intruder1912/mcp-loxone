...
use mcp_monitoring::MetricsCollector;
use mcp_protocol::*;
...
impl MiddlewareStack {
    ...
    pub async fn process_request(
        ...
        if let Some(security) = &self.security {
            let sec_context = mcp_security::middleware::RequestContext {
        ...
        if let Some(auth) = &self.auth {
            let auth_context = mcp_auth::manager::RequestContext {
                user_id: context.authenticated_user.clone(),
                roles: context.roles.iter().map(|_r| mcp_auth::models::Role::Admin).collect(), // TODO: proper role mapping
            };
            request = auth.process_request(request, &auth_context).await
                .map_err(|e| crate::handler::HandlerError::Authentication(e.to_string()))?;
        }
        
        // Monitoring middleware (last)
        if let Some(monitoring) = &self.monitoring {
            let mon_context = mcp_monitoring::collector::RequestContext {
                request_id: context.request_id,
            };
    ...
    pub async fn process_response(
        ...
        if let Some(auth) = &self.auth {
            ...
            response = auth.process_response(response, &auth_context).await
                .map_err(|e| crate::handler::HandlerError::Authentication(e.to_string()))?;
        }

...
use mcp_protocol::*;
use mcp_server::backend::{McpBackend, BackendError};
use std::sync::Arc;
...
impl From<LoxoneError> for Error {
    fn from(val: LoxoneError) -> Self {
        match val {
            LoxoneError::Connection(msg) => Error::internal_error(format!("Connection error: {}", msg)),
            LoxoneError::Authentication(msg) => Error::invalid_params(format!("Authentication error: {}", msg)),
            LoxoneError::Config(msg) => Error::invalid_params(format!("Configuration error: {}", msg)),
...
impl LoxoneBackend {
    ...
    fn get_cache_ttl(&self, uri: &str) -> Duration {
        match uri {
            ...
            "loxone://energy/consumption" | "loxone://weather/current" => {
...
impl McpBackend for LoxoneBackend {
    ...
    async fn health_check(&self) -> std::result::Result<(), Self::Error> {
        ...
        match self.server.client.health_check().await {
            ...
            Ok(false) => {
                ...
                Err(LoxoneError::connection("Health check failed: not connected"))
            }
            Err(e) => {
    ...
    async fn list_tools(&self, _params: PaginatedRequestParam) -> std::result::Result<ListToolsResult, Self::Error> {
    ...
    async fn call_tool(&self, params: CallToolRequestParam) -> std::result::Result<CallToolResult, Self::Error> {
        ...
        match adapters::handle_tool_call(&self.server, &params).await {
            ...
            Err(e) => {
                ...
                Ok(CallToolResult::error_text(format!("Tool execution failed: {}", e)))
            }
        }
    }

    async fn list_resources(&self, _params: PaginatedRequestParam) -> std::result::Result<ListResourcesResult, Self::Error> {
        ...
        let resources = vec![
            ...
            Resource {
                ...
                description: Some("Complete list of all devices with their current states".to_string()),
                mime_type: Some("application/json".to_string()),
                annotations: None,
                raw: None,
            },
    ...
    async fn read_resource(&self, params: ReadResourceRequestParam) -> std::result::Result<ReadResourceResult, Self::Error> {
        ...
        let (mime_type, content) = match params.uri.as_str() {
            ...
            "loxone://devices/category/lighting" => {
                let devices = self.server.context.devices.read().await;
                let lighting_devices: Vec<_> = devices.values()
                    .filter(|d| d.category == "lights" || d.device_type.contains("Light") || d.device_type.contains("Dimmer"))
                    .collect();
                ("application/json", serde_json::to_string(&lighting_devices)?)
            }
            "loxone://devices/category/blinds" => {
                let devices = self.server.context.devices.read().await;
                let blinds_devices: Vec<_> = devices.values()
            ...
            "loxone://devices/category/climate" => {
                let devices = self.server.context.devices.read().await;
                let climate_devices: Vec<_> = devices.values()
                    .filter(|d| d.category == "climate" || d.device_type.contains("Temperature") || d.device_type.contains("Climate"))
                    .collect();
            ...
            "loxone://audio/zones" => {
                ...
                let audio_devices: Vec<_> = devices.values()
                    .filter(|d| d.category == "audio" || d.device_type.contains("Audio") || d.device_type.contains("Music"))
                    .collect();
            ...
            "loxone://sensors/temperature" => {
                let devices = self.server.context.devices.read().await;
                let temp_sensors: Vec<_> = devices.values()
                    .filter(|d| d.device_type.contains("Temperature") || d.device_type.contains("Temp"))
                    .collect();
            ...
            "loxone://sensors/door-window" => {
                ...
                let door_window_sensors: Vec<_> = devices.values()
                    .filter(|d| d.device_type.contains("Door") || d.device_type.contains("Window") || d.device_type.contains("Contact"))
                    .collect();
                ("application/json", serde_json::to_string(&door_window_sensors)?)
            }
            "loxone://sensors/motion" => {
                let devices = self.server.context.devices.read().await;
                let motion_sensors: Vec<_> = devices.values()
            ...
            _ => {
                return Err(LoxoneError::not_found(format!("Resource not found: {}", params.uri)));
            }
        };
    ...
    async fn list_resource_templates(&self, _params: PaginatedRequestParam) -> std::result::Result<ListResourceTemplatesResult, Self::Error> {
    ...
    async fn list_prompts(&self, _params: PaginatedRequestParam) -> std::result::Result<ListPromptsResult, Self::Error> {
        info!("💬 Listing Loxone prompts");
        
        let prompts = vec![
            Prompt {
                name: "analyze_energy_usage".to_string(),
                description: Some("Analyze energy consumption patterns and provide optimization suggestions".to_string()),
                arguments: Some(vec![
                    PromptArgument {
            ...
            Prompt {
                ...
                description: Some("Generate a comprehensive summary of current home status".to_string()),
                arguments: Some(vec![
                    PromptArgument {
    ...
    async fn get_prompt(&self, params: GetPromptRequestParam) -> std::result::Result<GetPromptResult, Self::Error> {
    ...
    async fn subscribe(&self, params: SubscribeRequestParam) -> std::result::Result<(), Self::Error> {
    ...
    async fn unsubscribe(&self, params: UnsubscribeRequestParam) -> std::result::Result<(), Self::Error> {
        ...
        match params.uri.as_str() {
            ...
            _ => {
                ...
                Err(LoxoneError::not_found(format!("Resource not found for unsubscription: {}", params.uri)))
            }
        }
    }

    async fn complete(&self, params: CompleteRequestParam) -> std::result::Result<CompleteResult, Self::Error> {
        ...
        let completions = match params.ref_.as_str() {
            ...
            "device_types" => {
                vec!["Light".to_string(), "Jalousie".to_string(), "TimedSwitch".to_string(), "Dimmer".to_string()]
            }
            "sensor_types" => {
                vec!["temperature".to_string(), "humidity".to_string(), "motion".to_string(), "door_window".to_string()]
            }
            _ => {
        ...
        Ok(CompleteResult {
            completion: completions.into_iter().map(|c| CompletionInfo {
                completion: c,
                has_more: Some(false),
            }).collect(),
        })
    ...
    async fn set_level(&self, params: SetLevelRequestParam) -> std::result::Result<(), Self::Error> {
    ...
    async fn handle_custom_method(&self, method: &str, _params: serde_json::Value) -> std::result::Result<serde_json::Value, Self::Error> {

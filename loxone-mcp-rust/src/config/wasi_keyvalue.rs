...
mod wasi {
    ...
    impl Store {
            ...
            match storage.get_item(&prefixed_key) {
                Ok(Some(value)) => {
                    ...
                    let decoded = base64::engine::general_purpose::STANDARD
                        .decode(value)
                        .map_err(|e| LoxoneError::credentials(format!("Failed to decode value: {}", e)))?;
                    Ok(Some(decoded))
                }
                Ok(None) => Ok(None),
                Err(_) => Err(LoxoneError::credentials("Failed to access WASI keyvalue store")),
            }
        }
        ...
        async fn wasm_set(&self, key: &str, value: Vec<u8>) -> Result<()> {
            ...
            let prefixed_key = format!("wasi_kv:{}", key);
            storage.set_item(&prefixed_key, &encoded)
        ...
        async fn wasm_delete(&self, key: &str) -> Result<()> {
            ...
            storage.remove_item(&prefixed_key)
                .map_err(|_| LoxoneError::credentials("Failed to delete from WASI keyvalue store"))?;
...
impl WasiKeyValueManager {
        ...
        match self.store.get(&prefixed_key).await? {
            Some(bytes) => {
                let value = String::from_utf8(bytes)
                    .map_err(|e| LoxoneError::credentials(format!("Invalid UTF-8 in stored credential: {}", e)))?;
                Ok(Some(value))
            }
            None => Ok(None),
        }
    ...
    pub async fn set_credential(&self, key: &str, value: &str) -> Result<()> {
        let prefixed_key = format!("{}{}", self.prefix, key);
        self.store.set(&prefixed_key, value.as_bytes().to_vec()).await
    }
...
mod tests {
    ...
    async fn test_wasi_keyvalue_manager() {
        ...
        // Test set and get
        manager.set_credential("test_key", "test_value").await.unwrap();

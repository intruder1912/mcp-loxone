...
use crate::client::{LoxoneClient, LoxoneDevice, LoxoneResponse, LoxoneStructure, ClientContext};
...
impl LoxoneHttpClient {
    ...
    pub async fn new(config: LoxoneConfig, credentials: LoxoneCredentials) -> Result<Self> {
        ...
        let auth_header = format!("Basic {}", 
            base64::engine::general_purpose::STANDARD.encode(format!("{username}:{password}", username = credentials.username, password = credentials.password)));
...
impl LoxoneClient for LoxoneHttpClient {
    async fn connect(&mut self) -> Result<()> {
        ...
        match self.health_check().await {
            ...
            Ok(false) => {
                Err(LoxoneError::connection("Health check failed"))
            }
            Err(e) => {
    ...
    async fn get_structure(&self) -> Result<LoxoneStructure> {
        ...
        let structure: LoxoneStructure = serde_json::from_str(&text)
            .map_err(LoxoneError::Json)?;
        
        debug!("Structure loaded: {} controls, {} rooms", 

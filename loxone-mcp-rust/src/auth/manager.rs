...
use crate::auth::models::{ApiKey, AuthResult, AuthContext, Role, AuditEvent};
use crate::auth::storage::{StorageBackend, create_storage_backend, StorageBackendConfig};
use crate::auth::validation::{Validator, ValidationConfig, extract_client_ip, extract_api_key};
...
impl AuthenticationManager {
    ...
    pub async fn authenticate(&self, api_key_secret: &str, client_ip: &str) -> AuthResult {
        ...
        let result = self.validator.validate_authentication(api_key_secret, client_ip, &keys).await;
        
        match &result {
            AuthResult::Success(auth_success) => {
                debug!("Authentication successful for key: {} from IP: {}", auth_success.key.id, client_ip);
            ...
            AuthResult::RateLimited { retry_after_seconds } => {
                warn!("Rate limited IP {} for {} seconds", client_ip, retry_after_seconds);
            }
        }
    ...
    pub async fn create_key(
        ...
        // Log key creation
        self.log_audit_event(AuditEvent::key_created(
            key.id.clone(),
            created_by,
            role,
        )).await;

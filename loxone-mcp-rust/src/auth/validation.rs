...
use crate::auth::models::{ApiKey, AuthResult, AuthContext};
...
impl Validator {
    ...
    async fn record_failed_attempt(&self, client_ip: &str) {
        ...
        let state = rate_limits.entry(client_ip.to_string()).or_insert_with(|| {
            RateLimitState {
                failed_attempts: 0,
                window_start: now,
                blocked_until: None,
        ...
        let window_duration = chrono::Duration::minutes(self.config.failed_attempt_window_minutes as i64);
        if now - state.window_start > window_duration {
            ...
            if state.failed_attempts >= self.config.max_failed_attempts {
                let block_duration = chrono::Duration::minutes(self.config.block_duration_minutes as i64);
    ...
    fn find_key_by_secret<'a>(&self, secret: &str, keys: &'a HashMap<String, ApiKey>) -> Option<&'a ApiKey> {
...
mod tests {
    ...
    async fn test_rate_limiting() {
        ...
        for i in 1..=5 {
            let result = validator.validate_authentication(
                "invalid_key",
                "192.168.1.1",
                &keys,
            ).await;

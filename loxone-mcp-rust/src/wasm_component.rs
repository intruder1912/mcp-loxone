...
//! using the WIT bindings for WASI preview 2.
...
use crate::config::{
    credentials::{create_best_credential_manager, MultiBackendCredentialManager, LoxoneCredentials},
...
pub mod credential_manager_impl {
    ...
    pub async fn initialize(backend_config_json: &str) -> Result<()> {
        ...
        tracing::info!("Credential manager configured with backend: {:?}", backend_config);
        Ok(())
    }
    ...
    pub async fn store_credential(key: &str, value: &str) -> Result<()> {
        ...
        let credentials = LoxoneCredentials {
            username: if key == "LOXONE_USER" { value.to_string() } else { "admin".to_string() },
            password: if key == "LOXONE_PASS" { value.to_string() } else { "password".to_string() },
            api_key: if key == "LOXONE_API_KEY" { Some(value.to_string()) } else { None },
            #[cfg(feature = "crypto")]
            public_key: None,
        };
        ...
        let state_guard = COMPONENT_STATE.lock().unwrap();
        let state = state_guard.as_ref().unwrap();
        state.credential_manager.store_credentials(&credentials).await?;

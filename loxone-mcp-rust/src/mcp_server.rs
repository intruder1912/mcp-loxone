...
use rmcp::{
    ServiceExt, 
    ServerHandler, 
    model::{ServerInfo, CallToolResult, Content, ServerCapabilities},
};
...
impl LoxoneMcpServer {
    ...
    pub async fn new(mut config: ServerConfig) -> Result<Self> {
        ...
        let credential_manager = match crate::config::credentials::create_best_credential_manager().await {
            Ok(manager) => {
                info!("✅ Created multi-backend credential manager");
                manager
            }
            Err(e) => {
                error!("❌ Failed to create multi-backend credential manager: {}", e);
                error!("");
        ...
        let credentials = match credential_manager.get_credentials().await {
            ...
            Err(e) => {
                ...
                error!("");
                error!("3️⃣  Infisical (für Teams - keine extra Installation nötig!):");
                error!("    # Beispiel Setup (ersetze mit deinen Werten):");
                ...
                error!("    export INFISICAL_CLIENT_SECRET=\"st.abc123...\"  # Token von https://app.infisical.com");
                error!("    export INFISICAL_ENVIRONMENT=\"dev\"");
                error!("");
                error!("    Anleitung: ./INFISICAL_SETUP.md oder https://app.infisical.com/signup");
                error!("");
                error!("📚 Documentation: https://github.com/your-repo/loxone-mcp-rust#credentials");
                return Err(e);
            }
        };
        ...
        if let Ok(host) = std::env::var("LOXONE_HOST") {
            if let Ok(url) = host.parse() {
                ...
                warn!("⚠️ Failed to parse LOXONE_HOST environment variable: {}", host);
            }
        } else {
    ...
    pub async fn run_stdio(&self) -> Result<()> {
        ...
        let service = self.clone().serve((stdin(), stdout())).await
...
impl LoxoneMcpServer {
    ...
    pub async fn get_room_devices(&self, room_name: String) -> std::result::Result<CallToolResult, rmcp::Error> {
        ...
        let content = serde_json::to_string_pretty(&room_devices).unwrap_or_else(|_| "[]".to_string());
        Ok(CallToolResult::success(vec![Content::text(content)]))
    }
    
    /// Control a specific device
    pub async fn control_device(&self, device_id: String, action: String) -> std::result::Result<CallToolResult, rmcp::Error> {
        match self.client.send_command(&device_id, &action).await {
            Ok(_) => Ok(CallToolResult::success(vec![Content::text(
                format!("Successfully executed {} on device {}", action, device_id)
            )])),
            Err(e) => Ok(CallToolResult::error(vec![Content::text(
                format!("Failed to control device: {}", e)
            )])),
        }
    }
    ...
    pub async fn get_system_status(&self) -> std::result::Result<CallToolResult, rmcp::Error> {
        match self.client.health_check().await {
            Ok(true) => {
                ...
                let content = serde_json::to_string_pretty(&status).unwrap_or_else(|_| "{}".to_string());
                Ok(CallToolResult::success(vec![Content::text(content)]))
            },
            ...
            Err(e) => Ok(CallToolResult::error(vec![Content::text(
                format!("❌ System check failed: {}", e)
            )])),
        }
    }

    /// Enhanced get_room_devices with device type filtering
    pub async fn get_room_devices_enhanced(&self, room_name: String, device_type_filter: Option<String>) -> std::result::Result<CallToolResult, rmcp::Error> {
        let devices = self.context.devices.read().await;
        let rooms = self.context.rooms.read().await;
        
        // Find room by name
        let room_uuid = rooms.iter()
        ...
        if let Some(filter_type) = device_type_filter {
            room_devices.retain(|device| 
                device.device_type.to_lowercase().contains(&filter_type.to_lowercase())
            );
        }
    ...
    pub async fn control_device_enhanced(&self, device: String, action: String, room_hint: Option<String>) -> std::result::Result<CallToolResult, rmcp::Error> {
        let devices = self.context.devices.read().await;
        
        // Try to find device by UUID first, then by name
        let device_entry = devices.get(&device).cloned()
            .or_else(|| {
                devices.values()
                    .find(|d| {
                        ...
                        if let Some(ref room) = room_hint {
                            ...
                            name_match && d.room.as_ref().is_some_and(|r| {
                                // Check if room matches by name
                                let rooms = self.context.rooms.try_read();
                                if let Ok(rooms) = rooms {
                                    rooms.get(r).is_some_and(|room_obj| room_obj.name.to_lowercase() == room.to_lowercase())
                                } else {
                                    false
                                }
                            })
        ...
        if let Some(device_obj) = device_entry {
            match self.client.send_command(&device_obj.uuid, &action).await {
                ...
                Err(e) => Ok(CallToolResult::error(vec![Content::text(
                    format!("Failed to control device {}: {}", device_obj.name, e)
                )])),
            }
        } else {
    ...
    pub async fn control_all_rolladen(&self, action: String) -> std::result::Result<CallToolResult, rmcp::Error> {
        let devices = self.context.devices.read().await;
    ...
    pub async fn control_room_rolladen(&self, room_name: String, action: String) -> std::result::Result<CallToolResult, rmcp::Error> {
        let devices = self.context.devices.read().await;
        let rooms = self.context.rooms.read().await;
        ...
        if rolladen_devices.is_empty() {
            return Ok(CallToolResult::success(vec![Content::text(
                format!("No rolladen/blinds found in room '{}'", room_name)
            )]));
        }
    ...
    pub async fn control_all_lights(&self, action: String) -> std::result::Result<CallToolResult, rmcp::Error> {
        let devices = self.context.devices.read().await;
        let light_devices: Vec<_> = devices
            .values()
            .filter(|device| device.category == "lighting" || device.device_type == "Switch" || device.device_type == "Dimmer")
            .collect();
    ...
    pub async fn control_room_lights(&self, room_name: String, action: String) -> std::result::Result<CallToolResult, rmcp::Error> {
        let devices = self.context.devices.read().await;
        let rooms = self.context.rooms.read().await;
        ...
        let light_devices: Vec<_> = devices
            .values()
            .filter(|device| 
                (device.category == "lighting" || device.device_type == "Switch" || device.device_type == "Dimmer") &&
                (device.room.as_ref() == Some(&room_uuid))
            )
            .collect();
        ...
        if light_devices.is_empty() {
            return Ok(CallToolResult::success(vec![Content::text(
                format!("No lights found in room '{}'", room_name)
            )]));
        }
    ...
    pub async fn discover_all_devices(&self) -> std::result::Result<CallToolResult, rmcp::Error> {
        ...
        let device_list: Vec<_> = devices
            .values()
            .map(|device| {
                let room_name = device.room.as_ref()
    ...
    pub async fn get_devices_by_type(&self, device_type_filter: Option<String>) -> std::result::Result<CallToolResult, rmcp::Error> {
        let devices = self.context.devices.read().await;
        let rooms = self.context.rooms.read().await;
        ...
        if let Some(filter_type) = device_type_filter {
            let filtered_devices: Vec<_> = devices
                .values()
                .filter(|device| device.device_type.to_lowercase().contains(&filter_type.to_lowercase()))
                .map(|device| {
                    let room_name = device.room.as_ref()
                        .and_then(|room_uuid| rooms.get(room_uuid))
                        .map(|room| room.name.clone())
    ...
    pub async fn call_tool(&self, tool_name: &str, arguments: serde_json::Value) -> std::result::Result<serde_json::Value, String> {
        match tool_name {
            "list_rooms" => {
            ...
            "get_room_devices" => {
                let room_name = arguments.get("room_name")
                    .and_then(|v| v.as_str())
                    .ok_or("Missing room_name parameter")?;
                let device_type = arguments.get("device_type")
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string());
                match self.get_room_devices_enhanced(room_name.to_string(), device_type).await {
                    Ok(result) => self.convert_tool_result(result),
                    Err(e) => Err(format!("Failed to get room devices: {}", e))
                }
            },
            "control_device" => {
                let device = arguments.get("device")
                    .and_then(|v| v.as_str())
                ...
                let room = arguments.get("room")
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string());
                match self.control_device_enhanced(device.to_string(), action.to_string(), room).await {
                    Ok(result) => self.convert_tool_result(result),
            ...
            "control_room_rolladen" => {
                ...
                let action = arguments.get("action")
                    .and_then(|v| v.as_str())
                    .ok_or("Missing action parameter")?;
                match self.control_room_rolladen(room.to_string(), action.to_string()).await {
                    Ok(result) => self.convert_tool_result(result),
            ...
            "control_room_lights" => {
                ...
                let action = arguments.get("action")
                    .and_then(|v| v.as_str())
                    .ok_or("Missing action parameter")?;
                match self.control_room_lights(room.to_string(), action.to_string()).await {
                    Ok(result) => self.convert_tool_result(result),
                    Err(e) => Err(format!("Failed to control room lights: {}", e))
                }
            },
            "discover_all_devices" => {
                match self.discover_all_devices().await {
                    Ok(result) => self.convert_tool_result(result),
    ...
    fn convert_tool_result(&self, result: CallToolResult) -> std::result::Result<serde_json::Value, String> {
...
/// Manual ServerHandler implementation (temporary workaround for macro issues)
impl ServerHandler for LoxoneMcpServer {
        ...
        ServerInfo {
            ...
            instructions: Some(
                "Controls Loxone Generation 1 home automation systems. \
                 ...
                 and system monitoring capabilities.".into()
            ),
        }
    }
    ...
    async fn call_tool(
        ...
        match request.name.as_ref() {
            "list_rooms" => {
                self.list_rooms().await.map_err(|_| rmcp::Error::invalid_params("Failed to list rooms", None))
            },
            "get_room_devices" => {
                let args = request.arguments
                    .ok_or_else(|| rmcp::Error::invalid_params("Missing arguments", None))?;
                let room_name = args.get("room_name")
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string())
                    .ok_or_else(|| rmcp::Error::invalid_params("Missing room_name parameter", None))?;
                self.get_room_devices(room_name).await.map_err(|_| rmcp::Error::invalid_params("Failed to get room devices", None))
            },
            "control_device" => {
                let args = request.arguments
                    .ok_or_else(|| rmcp::Error::invalid_params("Missing arguments", None))?;
                ...
                let device_id = args.get("device_id")
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string())
                    .ok_or_else(|| rmcp::Error::invalid_params("Missing device_id parameter", None))?;
                    
                let action = args.get("action")
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string())
                    .ok_or_else(|| rmcp::Error::invalid_params("Missing action parameter", None))?;
                
                self.control_device(device_id, action).await.map_err(|_| rmcp::Error::invalid_params("Failed to control device", None))
            },
            "get_system_status" => {
                self.get_system_status().await.map_err(|_| rmcp::Error::invalid_params("Failed to get system status", None))
            },
            _ => Err(rmcp::Error::invalid_params("Unknown tool", None)),
        }
    }
}
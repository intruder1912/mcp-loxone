...
impl ConsentManager {
    ...
    pub async fn setup_channels(&mut self) -> (mpsc::UnboundedReceiver<ConsentRequest>, mpsc::UnboundedSender<ConsentResponse>) {
    ...
    pub async fn request_consent(&self, operation: OperationType, source: String) -> Result<ConsentDecision> {
        ...
        let request = self.create_consent_request(operation, sensitivity, source).await;
        ...
        if matches!(decision, ConsentDecision::Approved) {
            self.cache_consent_decision(&request.operation, &decision).await;
        }
        
        // Record the decision
        self.record_consent_decision(request, decision.clone()).await;
        ...
        if let Some(request) = pending.remove(&response.request_id) {
            let decision = if response.approved {
                ...
                ConsentDecision::Denied {
                    reason: response.reason.clone().unwrap_or_else(|| "User denied".to_string()),
                }
            };
            ...
            warn!("Received response for unknown consent request: {}", request_id);
        }
    ...
    async fn check_cached_consent(&self, operation: &OperationType) -> Option<ConsentDecision> {
        ...
        if let Some((decision, timestamp)) = cache.get(&operation_key) {
            let elapsed = SystemTime::now().duration_since(*timestamp).unwrap_or_default();
    ...
    fn generate_operation_description(&self, operation: &OperationType) -> (String, String, Vec<String>, String) {
        match operation {
            OperationType::DeviceControl { device_name, command, .. } => {
            ...
            OperationType::BulkDeviceControl { device_count, room_name, operation_type } => {
            ...
            OperationType::SecurityControl { action, scope } => {
                ...
                let details = format!("Execute security action '{}' with scope '{}'", action, scope);
                let risks = vec![
            ...
            OperationType::SystemConfiguration { setting, new_value, .. } => {
            ...
            OperationType::ConnectionManagement { action, target } => {
                ...
                let risks = vec![
                    "Connection state will change".to_string(),
                ];
    ...
    async fn record_consent_decision(&self, request: ConsentRequest, decision: ConsentDecision) {
        ...
        let decision_method = match &decision {
            ConsentDecision::Approved | ConsentDecision::Denied { .. } => DecisionMethod::UserDecision,
    ...
    pub async fn get_statistics(&self) -> ConsentStatistics {
        ...
        let approved = history.iter().filter(|r| matches!(r.decision, ConsentDecision::Approved)).count();
        let denied = history.iter().filter(|r| matches!(r.decision, ConsentDecision::Denied { .. })).count();
        let auto_approved = history.iter().filter(|r| matches!(r.decision, ConsentDecision::AutoApproved { .. })).count();
        let timed_out = history.iter().filter(|r| matches!(r.decision, ConsentDecision::TimedOut)).count();
...
pub trait ConsentProtected {
    ...
    async fn execute_with_consent<T>(
        ...
        source: String,
        executor: impl Fn() -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<T>> + Send>> + Send,
    ) -> Result<T>;
}

...
pub async fn get_climate_control(context: ToolContext) -> ToolResponse {
    ...
    let message = format!("Climate system: {} devices ({} room controllers, {} sensors, {} HVAC devices)",
...
pub async fn get_room_climate(
    ...
    let room_devices: Vec<_> = all_devices.into_iter()
    ...
    let climate_devices: Vec<ClimateDevice> = room_devices.into_iter()
        .map(parse_climate_device)
        .collect();
    ...
    let message = if let Some(ref controller) = room_controller {
        ...
        format!("Room '{}' climate: 1 controller, {} sensors{}", 
                room_name, sensors.len(), temp_info)
    } else {
        format!("Room '{}' climate: {} sensors (no controller)", room_name, sensors.len())
    };
...
pub async fn set_room_temperature(
    ...
    if !(5.0..=35.0).contains(&temperature) {
        return ToolResponse::error(format!("Invalid temperature {}°C. Must be between 5°C and 35°C", temperature));
    }
    ...
    let room_controller = all_devices.into_iter()
        .find(|device| {
    ...
    let controller = match room_controller {
        Some(device) => device,
        None => return ToolResponse::error(format!("No room controller found in room '{}'", room_name)),
    };
    
    // Send temperature set command
    let command = format!("setpoint/{}", temperature);
    
    let result = match context.send_device_command(&controller.uuid, &command).await {
        Ok(response) => {
            if response.code == 200 {
                ...
                return ToolResponse::error(format!(
                    "Failed to set temperature: response code {}", response.code
                ));
            }
        }
    ...
    ToolResponse::success_with_message(
        result,
        format!("Set target temperature to {:.1}°C for room '{}'", temperature, room_name)
    )
}
...
pub async fn get_temperature_readings(context: ToolContext) -> ToolResponse {
    ...
    for device in devices {
        ...
        if let Some(temp) = climate_device.current_temperature {
            ...
            if let Some(ref room) = climate_device.room {
                let current_max = room_temperatures.get(room).copied().unwrap_or(f64::NEG_INFINITY);
                if temp > current_max {
    ...
    // Calculate statistics
    let temperatures: Vec<f64> = readings.iter()
    ...
    ToolResponse::success_with_message(
        response_data,
        format!("Temperature readings from {} sensors (avg: {:.1}°C, range: {:.1}°C - {:.1}°C)",
                readings.len(), avg_temp, min_temp, max_temp)
    )
}
...
pub async fn set_room_mode(
    ...
    if !valid_modes.contains(&mode.as_str()) {
        return ToolResponse::error(format!(
            "Invalid mode '{}'. Valid modes: {}", mode, valid_modes.join(", ")
        ));
    }
    ...
    let result = match context.send_device_command(&controller.uuid, &command).await {
        Ok(response) => {
            if response.code == 200 {
                ...
                return ToolResponse::error(format!(
                    "Failed to set mode: response code {}", response.code
                ));
            }
        }
        Err(e) => return ToolResponse::error(format!("Failed to send command: {}", e)),
...
fn parse_climate_device(device: crate::client::LoxoneDevice) -> ClimateDevice {
    ...
    let current_temperature = device.states.get("tempActual")
        .or_else(|| device.states.get("temperature"))
        .or_else(|| device.states.get("value"))
        .and_then(|v| v.as_f64());
    
    let target_temperature = device.states.get("tempTarget")
        .or_else(|| device.states.get("setpoint"))
        .and_then(|v| v.as_f64());
    
    let mode = device.states.get("mode")

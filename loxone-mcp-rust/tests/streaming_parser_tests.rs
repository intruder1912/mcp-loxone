...
use loxone_mcp_rust::client::streaming_parser::{
    StreamingParserConfig, StreamingStructureParser, StructureSection, ParseProgress
};
...
async fn test_parse_progress_serialization() {
    ...
    assert_eq!(progress.completion_percentage, deserialized.completion_percentage);
...
async fn test_http_client_streaming_methods() {
    ...
    let result = tokio::time::timeout(
        ...
        client.get_structure_streaming()
    ).await;
...
async fn test_memory_estimation() {
    ...
    let estimated_size = 
        controls_count * 200 +     // ~200 bytes per control

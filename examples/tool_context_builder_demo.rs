...
use loxone_mcp_rust::tools::{ToolContextBuilder, ServerContextBuilderExt};
...
async fn example_handler_usage() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("\nNew way - using ToolContextBuilder:");
    {
        ...
        let elapsed = start.elapsed();
        info!("Created 3 contexts with 4 initial + 12 internal Arc clones in {:?}", elapsed);
...
impl loxone_mcp_rust::client::LoxoneClient for DemoMockClient {
    ...
    async fn send_command(&self, _uuid: &str, _command: &str) -> loxone_mcp_rust::Result<loxone_mcp_rust::client::LoxoneResponse> {
    ...
    async fn get_structure(&self) -> loxone_mcp_rust::Result<loxone_mcp_rust::client::LoxoneStructure> {
    ...
    async fn get_device_states(&self, _uuids: &[String]) -> loxone_mcp_rust::Result<std::collections::HashMap<String, serde_json::Value>> {
        Ok(std::collections::HashMap::new())
    }
    
    async fn get_state_values(&self, _state_uuids: &[String]) -> loxone_mcp_rust::Result<std::collections::HashMap<String, serde_json::Value>> {
        Ok(std::collections::HashMap::new())
    }
